...
# - Implements crossfade (video + audio) and a custom wipe transition (left_to_right) with audio crossfade
...
def make_text_overlay_clip(
    ...
    img_clip = ImageClip(rgb).set_mask(mask_clip).set_duration(duration)

    # Compute position
    ov_h, ov_w = rgb.shape[0], rgb.shape[1]
    x, y = compute_position_xy(position, ov_w, ov_h, w, h)
    img_clip = img_clip.set_position((x, y)).set_start(start)
    return img_clip
...
def make_wipe_mask_clip(duration, size, direction="left_to_right"):
    ...
    # Use VideoClip for time-varying mask (ImageClip cannot take a frame function)
    mask_clip = VideoClip(make_frame=make_frame, ismask=True, duration=d)
    return mask_clip
...
def assemble_timeline(clips, base_size, transitions):
    # clips: list of processed VideoFileClip-like objects (effects applied, resized, etc.)
    # transitions: list of dicts for between-clip transitions; len = len(clips)-1
    print("Assembling timeline with transitions...")
    timeline_elements = []
    t = 0.0  # start time of the "current" previous clip
    last_type = None
    ...
    for i, trans in enumerate(transitions):
        ...
        if ttype == "crossfade":
            ...
            # Video fade out of previous and fade in of next; audio crossfade too
            c_prev_v = c_prev.fx(vfx.fadeout, d).fx(afx.audio_fadeout, d).set_start(t)
            c_next_v = c_next.fx(vfx.fadein, d).fx(afx.audio_fadein, d).set_start(t + c_prev.duration - d)

            timeline_elements.append(c_prev_v)
            timeline_elements.append(c_next_v)

            # Update timeline: next clip starts at overlap start
            t = t + c_prev.duration - d
            last_type = "crossfade"
            ...
            # Previous clip continues; apply audio fadeout over d
            c_prev_a = c_prev.fx(afx.audio_fadeout, d).set_start(t)
            timeline_elements.append(c_prev_a)

            # Overlap start
            overlap_start = t + c_prev.duration - d
            mask = make_wipe_mask_clip(d, base_size, direction=direction)

            # Next clip audio fade-in; reveal with mask over d, then full after
            c_next_a = c_next.fx(afx.audio_fadein, d)
            c_next_overlap = c_next_a.subclip(0, min(d, c_next_a.duration)).set_mask(mask).set_start(overlap_start)
            timeline_elements.append(c_next_overlap)

            if c_next_a.duration > d:
                c_next_after = c_next_a.subclip(d, c_next_a.duration).set_start(t + c_prev.duration)
                timeline_elements.append(c_next_after)

            # Update timeline: next clip effectively starts at overlap_start
            t = overlap_start
            last_type = "wipe"

        elif ttype in ("cut", "none", "no", "straight"):
            print(f" - Transition {i+1}->{i+2}: cut")
            # Place previous clip fully, then advance
    ...
    # After processing transitions, ensure the final clip is present
    last_clip = clips[-1]
    if last_type in ("cut", "none", "no", "straight", "unknown", None):
        # Last clip hasn't been added yet
        timeline_elements.append(last_clip.set_start(t))
    # For crossfade/wipe, the last clip was already added in the loop
...
def main():
    ...
    overlay1 = make_text_overlay_clip(
        ...
        color="white",
        bg_color="black",
        bg_opacity=0.5,  # matches DEFAULT 0.5
        start=0.0,
        end=3.0,
    )
    ...
    # Audio: keep, but original_volume=0.0 -> mute, window 0.0..9.5
    if clip5.audio is not None:
        clip5 = apply_keep_audio(clip5, window_start=0.0, window_end=9.5, volume=0.0, fadein=0.0, fadeout=0.0)
    else:
        clip5 = clip5.set_audio(None)
    # Additional ops: rotate=90, scale=0.8
    # Workaround for MoviePy rotate bug with masked clips (ValueError: axes don't match array):
    # Remove mask before rotation since overlay is already composited and transparency isn't needed.
    clip5 = clip5.set_mask(None).rotate(90)
    ...
    # Determine FPS from first source clip if available, else default 30
