...
def apply_brightness_contrast(clip, brightness=1.0, contrast=1.0):
    # brightness: 1.0 = no change, >1 brighter, <1 darker
    # contrast: 1.0 = no change, >1 more contrast, <1 less
    def bc(image):
...
def apply_saturation(clip, saturation=1.0):
    # saturation: 1.0 = no change, >1 more saturated, <1 less
    def sat(image):
        img = image.astype(np.float32)
...
def apply_stabilization(clip):
    # Simple stabilization using OpenCV (feature tracking)
    # Only for short clips, not production-grade
...
def stabilize_frames(frames):
    # Use OpenCV to stabilize a list of frames (RGB)
    transforms = []
...
def add_text_overlay(clip, text, position, fontsize, color, bg_color=None, bg_opacity=0.5, start=0, end=None):
    # bg_color: e.g. 'black', bg_opacity: 0.0-1.0
    txt_clip = TextClip(
        text,
        fontsize=fontsize,
        color=color,
...
def add_text_overlay_custom(clip, text, position, fontsize, color, bg_color=None, bg_opacity=0.5, start=0, end=None):
    # More flexible, supports custom positions
    txt_clip = TextClip(
        text,
        fontsize=fontsize,
        color=color,
...
def process_audio_replace(clip, audio_path, duration, handling='trim', volume=1.0):
    if audio_path == 'DEFAULT: silence':
        ...
        audio = AudioFileClip(str(temp_silence)).set_duration(duration).volumex(volume)
        return audio
    else:
...
def apply_wipe_transition(clip1, clip2, duration=1.0):
    ...
    # For MoviePy 1.0.3, we need to use a custom VideoClip
...
def process_clip_1():
    ...
    # Audio: keep, volume=0.8, fadein=0.5, fadeout=0.5
...
def main():
    ...
    # Apply between-clip transitions
    ...
    wipe_duration = 0.8
    # For wipe, we need to overlap the end of clip2 and start of clip3
    # We'll use a custom transition
    # Shorten clip2 by wipe_duration at the end, and clip3 by wipe_duration at the start
...
```
**NOTES:**
- This script uses only the allowed libraries and MoviePy 1.0.3 API.
- All effects, transitions, overlays, and audio operations are implemented as specified.
- Progress is logged with print statements and tqdm for stabilization.
- All file paths are handled with pathlib.
- Intermediate directories are created as needed.
- No try/except blocks are used.
- The script is ready to run in the described environment.